...
def rgb_exclusion(image, channel):
    ...
    out = image.copy()
    if(channel=="B"):
        out[:,:,2]=np.zeros((image.shape[0],image.shape[1]))
        
    elif(channel=="G"):
        out[:,:,1]=np.zeros((image.shape[0],image.shape[1]))
        
    elif(channel=="R"):
        out[:,:,0]=np.zeros((image.shape[0],image.shape[1]))
    ### END YOUR CODE
...
def mix_images(image1, image2, channel1, channel2):
    ...
    x = rgb_exclusion(image1,channel1)
    y = rgb_exclusion(image2,channel2)
    
    out[:,:(image1.shape[1])//2,:]=x[:,:(image1.shape[1])//2,:]
    out[:,(image1.shape[1])//2:,:]=y[:,(image1.shape[1])//2:,:]
